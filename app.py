...
if uploaded is None:
    ...
    st.markdown('<div class="section-gap-lg"></div>', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Journal et rapport</div>', unsafe_allow_html=True)
    with st.expander("Afficher / masquer le journal"):
        audit_events = read_audit_events()
        if not audit_events:
            st.info("Le journal est vide pour l instant. Les actions (import, labels, entrainement, scoring) y seront tracees.")
        else:
            def summarize_details(details: dict) -> str:
            ...
            recent = audit_events[-50:]
            log_df = pd.DataFrame(
                [
                    {
                        "Horodatage": ev.get("timestamp", ""),
                        "Etape": ev.get("step", ""),
                        "Details": summarize_details(ev.get("details", {})),
                    }
                    for ev in recent
                ]
            )
            ...
            if st.button("Generer un rapport PDF/HTML du journal"):
                ...
                st.success(
                    f"Rapport audit sauvegarde : {report['html_path']} (HTML) et {report['pdf_path']} (PDF)"
                )
                st.download_button(
                    "Telecharger le rapport HTML",
                    data=report["html"].encode("utf-8"),
                    file_name=report["html_path"].name,
                    mime="text/html",
                )
                st.download_button(
